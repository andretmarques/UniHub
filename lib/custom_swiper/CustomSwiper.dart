...
class CustomSwiper extends StatefulWidget {
  ...
  @override
  State createState() => _CustomSwiperState();
}
...
class _CustomSwiperState extends State<CustomSwiper>
  ...
  void initState() {
    ...
    if (widget.controller != null) {
      widget.controller!.addListener(() {
        if (widget.controller!.state == CustomSwiperState.unswipe) {
          if (_lastCard != null) {
          ...
          } else if(widget.cards != null) {
            if(widget.cards!.isNotEmpty){
                if (widget.controller!.state == CustomSwiperState.left) {
                  _swipeHorizontal(context, true, CustomSwiperState.left);
                  _animationController.forward();
                } else if (widget.controller!.state == CustomSwiperState.right) {
                  _swipeHorizontal(context, true, CustomSwiperState.right);
                  _animationController.forward();
                }
              }
        }
  ...
  void _onEndAnimation() {
    if (_left < -widget.threshold || _left > widget.threshold) {
      _swipeHorizontal(context, false, null);
    } else {
      _goBack(context);
    }
  }
  ...
  void _swipeHorizontal(BuildContext context, bool custom, CustomSwiperState? direction) {
    var mediaQuery = MediaQuery.of(context).size.width;
    setState(() {
      _swipeTyp = 1;
      _leftAnimation = Tween<double>(
        begin: _left,
        end: custom ? direction == CustomSwiperState.left
            ? mediaQuery : -mediaQuery : (_left > widget.threshold)
            ? mediaQuery
            : -mediaQuery,
    ...
    if (custom) {
      switch (direction!) {
        case CustomSwiperState.left:
          _swipedDirectionHorizontal = 1;
          break;
        case CustomSwiperState.right:
          _swipedDirectionHorizontal = -1;
          break;
        case CustomSwiperState.unswipe:
          _swipedDirectionHorizontal = 1;
          break;
      }
    }
...
class CustomSwiperController extends ChangeNotifier {
  CustomSwiperState? state;

  //calls unswipe the card by changing the status of the controller
  void unswipe() {
    state = CustomSwiperState.unswipe;
    notifyListeners();
  }
  void swipeLeft() {
    state = CustomSwiperState.left;
    notifyListeners();
  }
  void swipeRight() {
    state = CustomSwiperState.right;
    notifyListeners();
  }
}

enum CustomSwiperState {
  unswipe,
  left,
  right
}
